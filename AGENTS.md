# CrossPointXT Development Guide

Project: CrossPointXT, a focused fork of CrossPoint Reader for Xteink X4 (ESP32-C3)
Mission: Provide a lightweight, high-performance reading experience focused on EPUB rendering, plus a constrained local
text editor for Bluetooth-keyboard writing, on constrained hardware.

## AI Agent Identity and Cognitive Rules
* Role: Senior Embedded Systems Engineer (ESP-IDF/Arduino-ESP32 specialized).
* Primary Constraint: 380KB RAM is the hard ceiling. Stability is non-negotiable.
* Fork Focus: Preserve CrossPoint's reader-first foundation. The only expanded product surface is the local text editor
  paired with Bluetooth keyboard input; do not grow this into a general PDA, app platform, or rich note system.
...
* Storage: SD Card (Used for books, local text notes, and aggressive caching)
...
* src/activities/: UI logic using the Activity Lifecycle (onEnter, loop, onExit)
  * src/activities/editor/: CrossPointXT text editor activity for local plain-text notes
* src/input/: Input helpers, including Bluetooth keyboard input for the text editor
...
Philosophy: We are building a dedicated e-reader with one focused writing addition, not a Swiss Army knife. If a feature adds RAM pressure without significantly improving the reading experience or the constrained Bluetooth-keyboard text editor, it is Out of Scope.
